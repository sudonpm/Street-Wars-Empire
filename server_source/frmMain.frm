...
Begin VB.Form frmMain 
   ...
   Caption         =   "Street Wars: Empire Server"
   ClientHeight    =   4560
   ClientLeft      =   495
   ClientTop       =   705
   ClientWidth     =   7560
   ...
   Begin VB.Menu mnuLogs 
      Caption         =   "&Logs"
      Begin VB.Menu mnuLogsUsers 
         Caption         =   "Users"
      End
      Begin VB.Menu mnuLogsGroups 
         Caption         =   "Groups"
      End
   End
...
If txtInput.Text <> "" Then
    For a = 1 To MaxUsers
        If User(a).Status = "Playing" Then
            frmMain.wsk(a).SendData Chr$(123) & "[Admin]-" & txtInput.Text & vbCrLf & Chr$(0)
            DoEvents
        End If
    Next a
    txtOutput.Text = txtOutput.Text & "[Admin]-" & txtInput.Text & vbCrLf
    txtInput.SetFocus
    txtInput.Text = ""
End If
...
Private Sub mnuLogsGroups_Click()

frmLogs.Show
DoEvents

frmLogs.lstIndex.AddItem "Connections"
frmLogs.lstIndex.AddItem "Disconnections"
frmLogs.lstIndex.AddItem "Spawned Items"
frmLogs.lstIndex.AddItem "Gets"
frmLogs.lstIndex.AddItem "Aims"
frmLogs.lstIndex.AddItem "Kills"
frmLogs.lstIndex.AddItem "Flees"
frmLogs.lstIndex.AddItem "Kicks"
frmLogs.lstIndex.AddItem "Bans"


End Sub

Private Sub mnuLogsUsers_Click()
Dim a As Integer
frmLogs.Show
DoEvents
...
For a = 0 To UBound(UserDB)
   ...
   frmLogs.lstIndex.AddItem UserDB(a).UName
   End If
Next a
...
'Spawn Drug Dealers/Druggies every Hour
   Call SpawnNPC(N_Dealer, 5) 'Spawn 5 Dealers in Each City
   Call SpawnNPC(N_Druggie, 5) 'Spawn 5 Druggies in Each city
   Call GenNpcInventory
   Call SpawnNPC(N_Cop, 2) 'Spawn 2 Police Officers
   Call SpawnNPC(N_Chief, 2) 'Spawn 2 Police Chiefs
   Call SpawnNPC(N_Detective, 2) 'Spawn 2 Detectives
...
If DealerNew - DealerOld > DealerSpawn Then
   Call SpawnNPC(N_Dealer, 5) 'Spawn 4 Dealers in Each City
   Call SpawnNPC(N_Druggie, 5) 'Spawn 4 Druggies in Each city
   Call GenNpcInventory
...
Dim ff As Integer 'Free File
ff = FreeFile
Open App.Path & "\logs\disconnects.log" For Append As ff
Print #ff, Date & " / " & Time & " : " & wsk(Index).RemoteHostIP & " " & User(Index).UName
Close ff
...
Dim ff As Integer 'Free File
ff = FreeFile
Open App.Path & "\logs\connections.log" For Append As ff
Print #ff, Date & " / " & Time & " : " & wsk(Index).RemoteHostIP
Close ff
End Sub
