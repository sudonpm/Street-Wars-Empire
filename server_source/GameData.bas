...
Public Const N_Chief = 7
Public Const N_Detective = 8
...
If Msg = SVersion Then
   frmMain.wsk(Index).SendData Chr$(2) & "Welcome to Street Wars: Empire." & vbCrLf & "Please report all bugs/problems to the admin at dream@dreamersway.com." & vbCrLf & "Duplicate accounts will not be tolerated." & vbCrLf & "Logging will not be tolerated." & vbCrLf & "Bots/Hacks will not be tolerated." & vbCrLf & "IP address will be banned for any of these." & vbCrLf & "Visit the Street Wars: Empire website at http://swe.dreamersway.com" & vbCrLf & "Warning your IP will be posted on our forums if you are caught cheating to share with other game servers!" & vbCrLf & vbCrLf & "Alright, what is your name?" & vbCrLf & vbCrLf & Chr$(0)
   DoEvents
   User(Index).Status = "GetName"
   Exit Sub
Else
   frmMain.wsk(Index).SendData Chr$(2) & "Your client version is outdated, please visit the Street Wars: Empire website and get the latest version at http://swe.dreamersway.com" & vbCrLf & vbCrLf & "Closing Connection..." & vbCrLf & vbCrLf & Chr$(0)
   DoEvents
...
Dim c As Integer, d As Integer, e
...
'save email
Dim fs As Integer 'Free File
fs = FreeFile
d = InStr(c + 1, Msg, Chr$(1))
Open App.Path & "\logs\emails.log" For Append As fs
Print #fs, Mid$(Msg, c + 1, d - c - 1)
Close fs
...
Select Case User(Index).Reputation
   ...
   Case 40001 To 80000
      User(Index).Rank = "Mafioso"
   Case 80001 To 140000
...
End Sub
Public Sub ShowCommands(Index As Integer)
Dim Msg As String 'Send String



Msg = "Welcome " & User(Index).UName & " for a list of commands type /help" & vbCrLf & Chr$(0)
frmMain.wsk(Index).SendData Chr$(2) & Msg
DoEvents
...
With ItemDB(45)
   ItemDB(45).IName = "Dual Desert Eagles"
   ItemDB(45).IDesc = "Two Desert Eagles. Packs quite a punch."
   ItemDB(45).ItemGUID = ""
   ...
   ItemDB(45).IType = C_Gun
   ItemDB(45).Price = 300000
   ItemDB(45).Multiple = False
   ItemDB(45).Movable = True
   ItemDB(45).CanBuy = 80001
   ItemDB(45).Decay = -1
   ItemDB(45).ForSale = True
End With

With ItemDB(46)
   ItemDB(46).IName = "M-16"
   ItemDB(46).IDesc = "Army issued M-16."
   ItemDB(46).ItemGUID = ""
   ...
   ItemDB(46).IType = C_Gun
   ItemDB(46).Price = 550000
   ItemDB(46).Multiple = False
   ItemDB(46).Movable = True
   ItemDB(46).CanBuy = 140001
   ItemDB(46).Decay = -1
   ItemDB(46).ForSale = True
End With

With ItemDB(47)
   ItemDB(47).IName = "Spike Bat"
   ItemDB(47).IDesc = "Ghetto and deadly."
   ItemDB(47).ItemGUID = ""
   ...
   ItemDB(47).IType = C_Melee
   ItemDB(47).Price = 30000
   ItemDB(47).Multiple = False
   ItemDB(47).Movable = True
   ItemDB(47).CanBuy = 80001
   ItemDB(47).Decay = -1
...
ElseIf User(Killed).Health <= 0 Then
   ...
   frmMain.wsk(Killed).SendData Chr$(252) & Chr$(231) & City(User(Killed).Location).Compass & Chr$(0)
   DoEvents
   ...
   PlayerKillPlayer = True
      ff = FreeFile
Open App.Path & "\logs\kills.log" For Append As ff
Print #ff, Date & " / " & Time & User(Index).UName & " killed " & User(Killed).UName
Close ff
End If
...
ElseIf Npc(User(Index).TargetNum).NHealth <= 0 Then
   ...
   ElseIf Npc(User(Index).TargetNum).NpcType = N_Cop Then
         ...
         User(Index).Kills = User(Index).Kills + 1
         Call UpdateGeneralInfo(Index)
   ElseIf Npc(User(Index).TargetNum).NpcType = N_Chief Then
         User(Index).Reputation = User(Index).Reputation + 10
         User(Index).Cash = User(Index).Cash + Npc(User(Index).TargetNum).NCash
         User(Index).Kills = User(Index).Kills + 1
         Call UpdateGeneralInfo(Index)
   ElseIf Npc(User(Index).TargetNum).NpcType = N_Detective Then
   ...
   ElseIf Npc(User(Index).TargetNum).NpcType = N_Hooker Then
         User(Index).Reputation = User(Index).Reputation + 3
         User(Index).Cash = User(Index).Cash + Npc(User(Index).TargetNum).NCash
         User(Index).Kills = User(Index).Kills + 1
         Call UpdateGeneralInfo(Index)
   End If
