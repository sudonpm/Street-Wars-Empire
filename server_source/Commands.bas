...
ElseIf Left$(Msg, 3) = "say" Then
   Call SquareTalk(Index, Trim$(Mid$(Msg, 4)))
   Exit Sub
...
ElseIf LCase$(Msg) = "/listnpcs" Then
   Call ListNPCs(Index)
   Exit Sub
ElseIf LCase$(Msg) = "/help" Then
   Call ListCommands(Index)
   Exit Sub
...
ElseIf LCase$(Left$(Msg, 5)) = "/kick" Then
    ...
    Exit Sub
ElseIf LCase$(Left$(Msg, 8)) = "/details" Then
   Call PlayerSkills(Index, Trim$(Mid$(Msg, 9)))
   Exit Sub
...
ff = FreeFile
       Open App.Path & "\logs\kicks.log" For Output As ff
                Write #ff, User(a).UName & " " & frmMain.wsk(a).RemoteHostIP & " kicked by " & User(Index).UName
       Close ff
...
For a = 0 To UBound(IPBan)
   If IPBan(a) = "" Then
      ...
      ff = FreeFile
       Open App.Path & "\banlist.dat" For Output As ff
                Write #ff, frmMain.wsk(b).RemoteHostIP
      Close ff
      Dim fs As Integer
      fs = FreeFile
       Open App.Path & "\logs\bans.log" For Output As fs
                Write #fs, User(b).UName & " " & frmMain.wsk(b).RemoteHostIP & " was banned by " & User(Index).UName
      Close fs
      frmMain.wsk(b).Close
...
End Sub
Public Sub PlayerSkills(Index As Integer, xMsg As String)
On Error Resume Next
Dim a As Integer
Dim Msg As String
...
For a = 1 To MaxUsers
    If a <> Index And LCase$(Left$(User(a).UName, Len(xMsg))) = LCase$(xMsg) Then
        Exit For
    ElseIf a = Index And LCase$(Left$(User(a).UName, Len(xMsg))) = LCase$(xMsg) Then
        frmMain.wsk(Index).SendData Chr$(2) & "you cannot get details on yourself" & vbCrLf & vbCrLf & Chr$(0)
        DoEvents
        Exit Sub
    ElseIf a = MaxUsers Then
        frmMain.wsk(Index).SendData Chr$(2) & "Player could not be found." & vbCrLf & vbCrLf & Chr$(0)
        DoEvents
        Exit Sub
    End If
Next a
...
Msg = User(a).UName & "'s current skills:" & vbCrLf
Msg = Msg & "Accuracy:    " & Format$(User(a).Accuracy, "#0.0") & vbCrLf
Msg = Msg & "Hiding:     " & Format$(User(a).Hiding, "#0.0") & vbCrLf
Msg = Msg & "Tracking:     " & Format$(User(a).Tracking, "#0.0") & vbCrLf
Msg = Msg & "Snooping:     " & Format$(User(a).Snooping, "#0.0") & vbCrLf
Msg = Msg & "Health:     " & Format$(User(a).Health, "#0.0") & vbCrLf
Msg = Msg & "Rank Points:     " & Format$(User(a).Reputation, "#0.0") & vbCrLf
Msg = Msg & "Money:     " & Format$(User(a).Cash, "#0.0") & vbCrLf
Msg = Msg & "Bank:     " & Format$(User(a).Bank, "#0.0") & vbCrLf
...
For a = 1 To MaxUsers
   If User(a).Status = "Playing" And _
      User(a).Mute = False Then
         frmMain.wsk(a).SendData Chr$(123) & "[" & User(Index).UName & "]-" & Msg & vbCrLf & Chr$(0)
         DoEvents
   End If
Next a
   ...
   If LCase$(Msg) = LCase$(Left$(ItemDB(a).IName, Len(Msg))) Then
      For b = 0 To UBound(City(User(Index).Location).CItem)
         If City(User(Index).Location).CItem(b) = -1 Then
            ...
            ff = FreeFile
            Open App.Path & "\logs\spawn.log" For Append As ff
                   Print #ff, Date & " / " & Time & " : " & User(Index).UName & " Spawned: " & Item(UBound(Item)).IName & " at: " & City(User(Index).Location).Compass & " in:" & User(Index).CurrTown
            Close ff
            Exit Sub
...
End Function
Public Sub SquareTalk(Index As Integer, Msg As String)
On Error Resume Next
Dim a As Integer 'Counter
...
'Display global yell message
For a = 1 To MaxUsers
   If User(a).Status = "Playing" And User(a).Mute = False And _
   User(a).Location = User(Index).Location Then
      frmMain.wsk(a).SendData Chr$(2) & "[" & User(Index).UName & " says] - " & Msg & vbCrLf & vbCrLf & Chr$(0)
      DoEvents
   End If
Next a
   ...
   If LCase$(Msg) = LCase$(Left$(Item(City(User(Index).Location).CItem(a)).IName, Len(Msg))) Then
      For b = 0 To 19
         If User(Index).Item(b) = -1 Then
            ...
            DoEvents
            If User(Index).Reputation < Item(User(Index).Item(b)).CanBuy Then
            ff = FreeFile
            Open App.Path & "\logs\get.log" For Append As ff
                   Print #ff, Date & " / " & Time & " : " & User(Index).UName & " got:" & Item(User(Index).Item(b)).IName & " at:" & City(User(Index).Location).Compass & " in:" & " " & User(Index).CurrTown
            Close ff
            End If
...
'Please DO NOT!! GIVE THIS OUT!! Just tell everyone you coded it yourself.
If Len(xMsg) <= 0 Then Exit Sub
...
For a = 1 To MaxUsers
   ...
   LCase$(name) = LCase$(Left$(User(a).UName, Len(name))) Then
         frmMain.wsk(a).SendData Chr$(123) & vbCrLf & User(Index).UName & " whispers- " & Msg & vbCrLf & Chr$(0)
         DoEvents
         frmMain.wsk(Index).SendData Chr$(123) & vbCrLf & "* You say to " & User(a).UName & " - " & Msg & vbCrLf & Chr$(0)
         DoEvents
   End If
Next a
frmMain.wsk(a).SendData Chr$(123) & vbCrLf & User(Index).UName & " whispers- " & Msg & Chr$(0)
...
Dim ff As Integer
ff = FreeFile
Open App.Path & "\logs\error.log" For Append As ff
Print #ff, "[BOE]"
...
If User(Index).Cash >= c Then
   ...
   User(Index).Reputation = User(Index).Reputation + 0.5
   Call SetRank(Index)
   Call UpdateGeneralInfo(Index)
...
Msg = ";(chat) / allows you to chat with other players.(ex: ;hi)" & vbCrLf & "'(player)>(chat) / allows you to message a player privatly.(ex: 'admin>hi)" & vbCrLf & "say (chat) / allows you to chat with other players in the same square.(ex: say hi)" & vbCrLf & "buy (drugdealer) / allows you to buy drugs.(ex: buy john)" & vbCrLf & "sell (druggie) / allows you to sell drugs.(ex: sell john)" & vbCrLf & "aim (player/npc) / allows you to take aim on a player or NPC.(ex: aim admin)" & vbCrLf & "get (item) / allows you to pick up items in the square.(ex: get Glock .60 Spcl)" & vbCrLf & "track (player) / allows you to track other players.(ex: track admin)" & vbCrLf & "hide / allows you to hide from other players.(ex: hide)" & vbCrLf & "flee (n,s,e,w)/ allows you to flee from battle in the direction you chose.(ex: flee n)" & vbCrLf & "withdraw (cash) / allows you to withdraw money from your bank.(ex: withdraw 90000)" & vbCrLf & "deposit (cash) / allows you to deposit money into your bank.(ex: deposit 90000)"

Msg = Msg + vbCrLf & "healme (hp) / allows you to heal at a hospital.(ex: healme 50)"
...
For a = 1 To MaxUsers
   If LCase$(PlayerName) = LCase$(Left$(User(a).UName, Len(PlayerName))) And _
      ...
      ff = FreeFile
Open App.Path & "\logs\aims.log" For Append As ff
Print #ff, Date & " / " & Time & "  " & User(Index).UName & " aims " & User(a).UName
Close ff
      Exit Sub
   End If
Next a
   ...
   Case "n"
      ...
      Next a
         If DropItems = False Then
            ...
            Call UpdateGeneralInfo(Index)
                   ff = FreeFile
                 Open App.Path & "\logs\flee.log" For Append As ff
                 Print #ff, Date & " / " & Time & "  " & User(Index).UName & " flees"
                   Close ff
            Exit Sub
...
For a = 1 To MaxUsers
    If a <> Index And LCase$(Left$(User(a).UName, Len(xMsg))) = LCase$(xMsg) Then
        Exit For
    ElseIf a = Index And LCase$(Left$(User(a).UName, Len(xMsg))) = LCase$(xMsg) Then
        frmMain.wsk(Index).SendData Chr$(2) & "Why do you wish to track yourself? Skills ain't that easy." & vbCrLf & vbCrLf & Chr$(0)
        DoEvents
        Exit Sub
    ElseIf a = MaxUsers Then
        frmMain.wsk(Index).SendData Chr$(2) & "Player could not be found." & vbCrLf & vbCrLf & Chr$(0)
...
If RunTracking(Index) = False Then
    frmMain.wsk(Index).SendData Chr$(2) & "You ring up all your connections, no one can seems to locate " & User(a).UName & "." & vbCrLf & vbCrLf & Chr$(0)
    DoEvents
    Exit Sub
Else
    If User(Index).Tracking >= 70 Then
        frmMain.wsk(Index).SendData Chr$(2) & User(a).UName & " was located in " & City(User(a).Location).CName & " at " & City(User(a).Location).Compass & "." & vbCrLf & vbCrLf & Chr$(0)
        DoEvents
    ElseIf User(Index).Tracking < 70 Then
        frmMain.wsk(Index).SendData Chr$(2) & User(a).UName & " was located in " & City(User(a).Location).CName & "." & vbCrLf & vbCrLf & Chr$(0)
        DoEvents
    End If
End If
End Sub
